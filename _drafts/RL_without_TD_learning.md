#public #blogger
blogger_updated: Sat, 01 Nov 2025 02:00:00 -0700
...
![](https://bair.berkeley.edu/static/blog/rl-without-td-learning/teaser_short.png)
...
\[\begin{aligned} Q(s, a) \gets r + \gamma \max\_{a'} Q(s', a'), \end{aligned}\]
...
\[\begin{aligned} Q(s\_t, a\_t) \gets \sum\_{i=0}^{n-1} \gamma^i r\_{t+i} + \gamma^n \max\_{a'} Q(s\_{t+n}, a'). \end{aligned}\]
...
![](https://bair.berkeley.edu/static/blog/rl-without-td-learning/teaser.png)
...
\max\_{w \in \mathcal{S}} V(s, w)V(w, g) & \text{otherwise}
\end{cases}
\end{aligned}\]
...
Here’s our key idea: we *restrict* the search space of $w$ to the states that appear in the dataset, specifically, those that lie between $s$ and $g$ in the dataset trajectory. Also, instead of searching for the optimal $\text{argmax}\_w$, we compute a “soft” $\text{argmax}$ using [expectile regression](https://arxiv.org/abs/2110.06169). Namely, we minimize the following loss:

\[\begin{aligned} \mathbb{E}\left[\ell^2\_\kappa (V(s\_i, s\_j) - \bar{V}(s\_i, s\_k) \bar{V}(s\_k, s\_j))\right], \end{aligned}\]

where $\bar{V}$ is the target value network, $\ell^2\_\kappa$ is the expectile loss with an expectile $\kappa$, and the expectation is taken over all $(s\_i, s\_k, s\_j)$ tuples with $i \leq k \leq j$ in a randomly sampled dataset trajectory.
...
](https://bair.berkeley.edu/static/blog/rl-without-td-learning/humanoidmaze.mp4)
...
Your browser does not support the video tag.
](https://bair.berkeley.edu/static/blog/rl-without-td-learning/puzzle.mp4)
...
![](https://bair.berkeley.edu/static/blog/rl-without-td-learning/table.png)
...
![](https://bair.berkeley.edu/static/blog/rl-without-td-learning/1b.svg)
